...
func main() {
	...
	var descrip, aniomesDev, aniomesPag string
	...
	if err := db.Model(&models.DetallesHab{}).Select("DISTINCT tipo_reg, mes, anio, codigo_tipo_planta").Scan(&uniqueRecords).Error; err != nil {
	...
	for _, record := range uniqueRecords {
		...
		liqui := models.Liquis{
			...
			Anio_liq:         uint(record.Anio),
			Mes_liq:          uint(record.Mes),
			Anio_dev:         uint(record.Anio),
			Mes_dev:          uint(record.Mes),
			Anio_gan:         uint(record.Anio),
			Mes_gan:          uint(record.Mes),
